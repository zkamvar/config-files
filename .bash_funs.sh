...
function stroll() {
  echo -n "ᕕ( ᐛ )ᕗ" |
    (xclip -selection clipboard 2>/dev/null || pbcopy)
  echo "ᕕ( ᐛ )ᕗ copied to your clipboard"
}
export -f stroll
...
function jkl {
  echo -n "(屮ﾟДﾟ)屮" |
    (xclip -selection clipboard 2>/dev/null || pbcopy)
  echo "(屮ﾟДﾟ)屮 copied to your clipboard"
}
export -f jkl
...
function concern {
  echo -n "ಠ_ಠ" |
    (xclip -selection clipboard 2>/dev/null || pbcopy)
  echo "ಠ_ಠ copied to your clipboard"
}
export -f concern
...
function vpn_choose() {
  ...
  if containsElement "${declared}" "${connections[@]}"; then
    echo "${declared}"
    return 0
  fi

  # https://linuxize.com/post/bash-select/
  select con in "${connections[@]}":; do
    echo "${con}"
    return 0
  done
